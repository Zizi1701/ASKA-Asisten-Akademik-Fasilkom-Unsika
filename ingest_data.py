...
def create_vector_db():
    ...
    # Pastikan nama file di bawah ini sesuai persis dengan yang ada di folder data/ Anda
    file_configs = [
        {
    ...
    # 1. Splitter untuk data yang sudah diratakan (Row-to-Text)
    # Memotong murni berdasarkan baris baru (\n) agar data dosen/jadwal tidak terpotong di tengah kalimat
    ...
    # 2. Splitter untuk dokumen panduan naratif
    # Memotong dengan memperhatikan hierarki bab, sub-bab, dan paragraf
    ...
    for config in file_configs:
        ...
        try:
            # Menggunakan TextLoader spesifik per file
            loader = TextLoader(filepath, encoding='utf-8')
            documents = loader.load()
            
            # Injeksi Metadata (Sangat penting agar LLM dan sistem tahu asal usul konteks)
            for doc in documents:
                doc.metadata["kategori"] = config["kategori"]
                doc.metadata["source"] = config["filename"]
                
            # Terapkan splitter sesuai tipe data
    ...
    print("3. Membuat embeddings (proses ini mungkin memakan waktu)...")
    ...
    print("✅ Selesai! Vectorstore berhasil menyimpan semua dokumen dengan konfigurasi spesifik.")
